...
static void keyboardHandler(int vector) {
    ...
    u16 scancode = ReadByte(KeyboardDataPort);  // 从键盘数据寄存器中读取一个扫描码（编码）
    u8 ext = 2;
...
Size ReadLine(char* buf, Size len) {
    ...
    while (
        globalBufferLength == 0 || 
        globalBuffer[globalBufferLength - 1] != '\n'
    );

    if (globalBufferLength > len) {
        PrintWithColor(RED, "Your input is too big, the length is lt %d, please trying input again\n", len);
        globalBuffer = NULL;
        globalBufferLength = 0;
        RestoreInterruptStatus(status);
