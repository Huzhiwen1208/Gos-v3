...
Boolean WriteFileContent(String path, String content, Boolean trunc) {
    ...
    DiskCacheRead(blockID, buffer);
    if (size == 0)
        buffer[0] = (u8)EOF, fileInode->Size = size = 1;
    MemoryCopy(buffer + size % 512 - 1, content, StringLength(content));
    buffer[size % 512 - 1 + StringLength(content)] = (u8)EOF;
